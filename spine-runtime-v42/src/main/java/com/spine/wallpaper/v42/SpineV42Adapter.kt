package com.spine.wallpaper.v42
...
/**
 * Isolated Spine 4.2 Runtime Adapter.
 * 使用官方 spine-libgdx 4.2.12 runtime（通过 shadow plugin 重定位到 com.spine.wallpaper.spine42）。
 * 支持物理约束、序列帧附件与独立缩放/旋转时间轴。
 */
class SpineV42Adapter(
    ...
    override val runtimeVersion: SpineVersion = SpineVersion.V42
    ...
    override fun update(deltaSeconds: Float) {
        ...
        // 4.2: updateWorldTransform 需要 Physics 参数
    ...
    override fun hitTest(screenX: Float, screenY: Float): String? = "v42_body"
...
private class PeekAttachmentLoader : AttachmentLoader {
    override fun newRegionAttachment(skin: Skin, name: String, path: String, sequence: Sequence?): RegionAttachment {
        return RegionAttachment(name).apply { setRegion(TextureRegion()) }
    }
    override fun newMeshAttachment(skin: Skin, name: String, path: String, sequence: Sequence?): MeshAttachment {
