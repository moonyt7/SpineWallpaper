...
class Skin38(val name: String) {
    ...
    fun setAttachment(slotIndex: Int, name: String, attachment: Attachment38) {
        ...
        if (name != attachment.name) {
            attachments[SkinKey38(slotIndex, attachment.name)] = attachment
        }
    }

    fun getAttachment(slotIndex: Int, name: String): Attachment38? {
        val direct = attachments[SkinKey38(slotIndex, name)]
        if (direct != null) return direct
        for ((key, att) in attachments) {
            if (key.slotIndex == slotIndex && (key.name.equals(name, ignoreCase = true) || att.name.equals(name, ignoreCase = true) ||
                (att as? RegionAttachment38)?.path?.equals(name, ignoreCase = true) == true ||
                (att as? MeshAttachment38)?.path?.equals(name, ignoreCase = true) == true)) {
                return att
            }
        }
        for ((key, att) in attachments) {
            if (key.name.equals(name, ignoreCase = true) || att.name.equals(name, ignoreCase = true)) {
                return att
            }
        }
        return attachments.entries.firstOrNull { it.key.slotIndex == slotIndex }?.value
    }
}
...
class MeshAttachment38(name: String) : Attachment38(name) {
    ...
    var inheritDeform: Boolean = true
...
class SkeletonRenderer38 {
    ...
    fun draw(batch: PolygonSpriteBatch, skeleton: Skeleton38) {
        ...
        for (slot in drawOrder) {
            ...
            val a = if (skelColor.a * slotColor.a <= 0.001f) 1f else (skelColor.a * slotColor.a).coerceIn(0f, 1f)
            ...
            if (attachment is RegionAttachment38) {
                ...
                val attA = if (attachment.color.a <= 0.001f) 1f else attachment.color.a
                val finalA = (a * attA).coerceIn(0f, 1f)
                val finalR = (r * attachment.color.r).coerceIn(0f, 1f)
                val finalG = (g * attachment.color.g).coerceIn(0f, 1f)
                val finalB = (b * attachment.color.b).coerceIn(0f, 1f)

                val cFloat = if (pma) {
                    colorToFloatBits(finalR * finalA, finalG * finalA, finalB * finalA, finalA)
                } else {
                    colorToFloatBits(finalR, finalG, finalB, finalA)
                }
                ...
                if (region == null) continue
                if (attachment.uvs.size != attachment.regionUVs.size) {
                    attachment.updateUVs()
                }
...
data class LinkedMesh38(
    ...
    val inheritDeform: Boolean = true
)
...
class SkeletonBinary38(val atlas: TextureAtlas) {
    ...
    fun readSkeletonData(file: FileHandle): SkeletonData38 {
        ...
        try {
            if (bytes.size > 12) {
                ...
                val v38 = testReader38.readRawString()
                if (v38 != null && v38.length in 3..14 && (v38.startsWith("3.8") || v38.startsWith("4."))) {
                    ...
                    testReader37.readRawString() // string hash in 3.6/3.7
                    val v37 = testReader37.readRawString()
        ...
        if (isSpine38) {
            ...
            val version = reader.readRawString() ?: "3.8.99"
            ...
            if (nonessential) {
                ...
                data.imagesPath = reader.readRawString()
                data.audioPath = reader.readRawString()
            }
        } else {
            data.hash = reader.readRawString()
            data.version = reader.readRawString() ?: versionStr
            data.width = reader.readFloat() * scale
            data.height = reader.readFloat() * scale
            nonessential = reader.readBoolean()
            if (nonessential) {
        ...
        // 2. Strings Table (Spine 3.8 binary stores strings table right after header; Spine 3.6/3.7 does not)
        val strings: Array<String?>
        val hasStringsTable: Boolean
        if (isSpine38) {
            val stringCount = reader.readVarint(true)
            strings = Array(stringCount) { null }
            for (i in 0 until stringCount) {
                strings[i] = reader.readRawString()
            }
            hasStringsTable = true
        } else {
            strings = emptyArray()
            hasStringsTable = false
        }

        fun readStringRef(): String? {
            return if (hasStringsTable) {
                val index = reader.readVarint(true)
                if (index == 0) null else if (index - 1 in strings.indices) strings[index - 1] else null
            } else {
        ...
        for (i in 0 until boneCount) {
            ...
            val bone = BoneData38(i, name, parent).apply {
                ...
                val modeIdx = reader.readVarint(true).coerceIn(0, 4)
                transformMode = TransformMode38.values()[modeIdx]
        ...
        if (data.bones.isEmpty()) {
            data.bones.add(BoneData38(0, "root", null))
        }

        // 4. Slots
        val slotCount = reader.readVarint(true)
        for (i in 0 until slotCount) {
            val slotName = readStringRef() ?: ""
            ...
            val slotData = SlotData38(i, slotName, boneData).apply {
                ...
                attachmentName = readStringRef()
                val blendIdx = reader.readVarint(true).coerceIn(0, 3)
                blendMode = BlendMode38.values()[blendIdx]
            }
        ...
        try {
            ...
            for (i in 0 until ikCount) {
                val ik = IkConstraintData38(readStringRef() ?: "").apply {
        ...
        try {
            ...
            for (i in 0 until tfCount) {
                val tf = TransformConstraintData38(readStringRef() ?: "").apply {
                    order = reader.readVarint(true)
                    skinRequired = if (isSpine38) reader.readBoolean() else false
                    val bonesSize = reader.readVarint(true)
                    for (b in 0 until bonesSize) {
        ...
        try {
            ...
            for (i in 0 until pcCount) {
                val pc = PathConstraintData38(readStringRef() ?: "").apply {
                    order = reader.readVarint(true)
                    skinRequired = if (isSpine38) reader.readBoolean() else false
                    val bonesSize = reader.readVarint(true)
                    for (b in 0 until bonesSize) {
        ...
        // 8. Default Skin
        val defaultSkin = readSkin(reader, "default", true, nonessential, isSpine38, hasStringsTable, strings)
        ...
        for (i in 0 until skinCount) {
            val skin = readSkin(reader, "", false, nonessential, isSpine38, hasStringsTable, strings)
        ...
        try {
            ...
            for (i in 0 until eventCount) {
                val ev = EventData38(readStringRef() ?: "").apply {
        ...
        try {
            ...
            for (i in 0 until animationCount) {
                val animName = readStringRef() ?: "anim_$i"
                try {
                    val anim = readAnimation(reader, animName, data, isSpine38, hasStringsTable, strings)
        ...
        if (data.animations.isEmpty()) {
            data.animations.add(Animation38("idle", 1.0f))
        }

        // Fallback: If no skin/attachments were created, synthesize from atlas
        if (data.defaultSkin == null && data.skins.isEmpty()) {
            val synthSkin = Skin38("default")
            if (data.slots.isEmpty()) {
                val rootBone = data.bones.firstOrNull() ?: BoneData38(0, "root", null)
                val slot = SlotData38(0, "main", rootBone)
                data.slots.add(slot)
            }
            val mainSlot = data.slots.first()
            for (reg in atlas.regions) {
                ...
                synthSkin.setAttachment(mainSlot.index, rName, att)
                if (mainSlot.attachmentName == null) mainSlot.attachmentName = rName
            }
            data.defaultSkin = synthSkin
            data.skins.add(synthSkin)
        }

        return data
    }

    private fun readSkin(
        reader: BinaryStreamReader38,
        skinName: String,
        isDefaultSkin: Boolean,
        nonessential: Boolean,
        isSpine38: Boolean,
        hasStringsTable: Boolean,
        ...
        val name = if (isDefaultSkin) "default" else (readStringRef() ?: skinName)
        val slotCount = reader.readVarint(true)
        if (slotCount == 0) return null
        val skin = Skin38(name)

        for (i in 0 until slotCount) {
            val slotIndex = reader.readVarint(true)
            val attachmentCount = reader.readVarint(true)
            for (ii in 0 until attachmentCount) {
                val placeholderName = readStringRef() ?: ""
                val attachment = readAttachment(reader, slotIndex, placeholderName, nonessential, hasStringsTable, strings)
                if (attachment != null) {
    ...
    private fun readAttachment(
        ...
        val name = readStringRef() ?: placeholderName
        when (type) {
            0 -> { // Region
                val path = readStringRef() ?: name
            ...
            3 -> { // LinkedMesh
                val path = readStringRef() ?: name
                val colorInt = reader.readInt()
                val skinName = readStringRef()
                val parentName = readStringRef()
        ...
        for (i in 0 until deformCount) {
            ...
            for (ii in 0 until slotCountD) {
                ...
                for (iii in 0 until timelineCount) {
                    val attachmentName = readStringRef() ?: ""
                    val frameCount = reader.readVarint(true)
...
class SkeletonBinary4x(val atlas: TextureAtlas) {
    var scale: Float = 1.0f
    private val linkedMeshes = mutableListOf<LinkedMesh38>()

    fun readSkeletonData(file: FileHandle): SkeletonData38 {
        val bytes = file.readBytes()
        val data = SkeletonData38().apply { this.atlas = this@SkeletonBinary4x.atlas }
        linkedMeshes.clear()
        ...
        val hashLong = reader.readLong()
        data.hash = if (hashLong != 0L) hashLong.toString() else null
        val version = reader.readRawString() ?: "4.1.00"
        data.version = version
        val isVersion41Plus = version.startsWith("4.1") || version.startsWith("4.2")
        ...
        // 2. Strings Table
        val stringCount = reader.readVarint(true)
        val strings = Array<String?>(stringCount) { null }
        for (i in 0 until stringCount) {
            strings[i] = reader.readRawString()
        }
        ...
        fun readStringRef(): String? {
            val index = reader.readVarint(true)
            return if (index == 0) null else if (index - 1 in strings.indices) strings[index - 1] else null
        }
        ...
        for (i in 0 until boneCount) {
            ...
            val bone = BoneData38(i, name, parent).apply {
                ...
                val modeIdx = reader.readVarint(true).coerceIn(0, 4)
                transformMode = TransformMode38.values()[modeIdx]
                skinRequired = reader.readBoolean()
                if (nonessential) reader.readInt()
            }
            data.bones.add(bone)
        }
        ...
        try {
            ...
            for (i in 0 until ikCount) {
                val ik = IkConstraintData38(readStringRef() ?: "").apply {
                    ...
                    target = if (targetIdx in data.bones.indices) data.bones[targetIdx] else (data.bones.firstOrNull() ?: BoneData38(0, "root", null))
                    mix = reader.readFloat()
                    softness = reader.readFloat() * scale
                    bendDirection = reader.readByte().toInt()
        ...
        // 8. Default Skin
        try {
            val defaultSkin = readSkin4x(reader, "default", true, nonessential, strings, isVersion41Plus)
            data.defaultSkin = defaultSkin
            if (defaultSkin != null) data.skins.add(defaultSkin)
        } catch (_: Throwable) {}
        ...
        try {
            val skinCount = reader.readVarint(true)
            for (i in 0 until skinCount) {
                val skin = readSkin4x(reader, "", false, nonessential, strings, isVersion41Plus)
                if (skin != null) data.skins.add(skin)
            }
        } catch (_: Throwable) {}
        ...
        // 11. Animations
        val animNames = LinkedHashSet<String>()
        val knownNames = HashSet<String>()
        for (b in data.bones) knownNames.add(b.name)
        for (s in data.slots) {
            knownNames.add(s.name)
            s.attachmentName?.let { knownNames.add(it) }
        }
        for (ik in data.ikConstraints) knownNames.add(ik.name)
        for (tf in data.transformConstraints) knownNames.add(tf.name)
        for (pc in data.pathConstraints) knownNames.add(pc.name)
        for (sk in data.skins) {
            knownNames.add(sk.name)
            for (att in sk.attachments.values) {
                knownNames.add(att.name)
                (att as? RegionAttachment38)?.path?.let { knownNames.add(it) }
                (att as? MeshAttachment38)?.path?.let { knownNames.add(it) }
            }
        }
        for (ev in data.events) knownNames.add(ev.name)
        ...
        try {
            ...
            for (i in 0 until animationCount) {
                val animName = readStringRef() ?: "anim_$i"
                try {
                    val anim = readAnimation4x(reader, animName, isVersion41Plus, strings, data)
                    data.animations.add(anim)
        ...
        // If animation parsing stopped early or missed strings, scan strings table for remaining animation names
        if (data.animations.size <= 1) {
            for (str in strings) {
                if (str != null && str.isNotEmpty() && !knownNames.contains(str) && !animNames.contains(str)) {
                    if (str.contains("idle", ignoreCase = true) || str.contains("walk", ignoreCase = true) ||
                        str.contains("run", ignoreCase = true) || str.contains("attack", ignoreCase = true) ||
                        str.contains("touch", ignoreCase = true) || str.contains("action", ignoreCase = true) ||
                        str.contains("anim", ignoreCase = true) || str.contains("motion", ignoreCase = true) ||
                        str.contains("stand", ignoreCase = true) || str.contains("pose", ignoreCase = true)) {
                        data.animations.add(Animation38(str, 1.0f))
                        animNames.add(str)
                    }
                }
            }
    ...
    private fun readAnimation4x(
        reader: BinaryStreamReader38,
        name: String,
        isVersion41Plus: Boolean,
        strings: Array<String?>,
        data: SkeletonData38
    ): Animation38 {
        fun readStringRef(): String? {
        ...
        for (i in 0 until timelineCount) {
            ...
            when (type) {
                ...
                1, 2, 3, 4, 5, 6 -> { // Color / RGB / Alpha / TwoColor
                    val slotIndex = reader.readVarint(true)
                    val frameCount = reader.readVarint(true)
                    val bezierCount = reader.readVarint(true)
                    val valueFloats = when (type) {
                        1 -> 4
                        2 -> 3
                        3 -> 1
                        4 -> 7
                    ...
                    for (f in 0 until frameCount) {
                        val time = reader.readFloat()
                        for (v in 0 until valueFloats) reader.readFloat()
                        if (time > maxDuration) maxDuration = time
                    }
                    readBeziers(reader, bezierCount, isVersion41Plus)
                }
                7 -> { // Rotate
                    val boneIndex = reader.readVarint(true)
                    val frameCount = reader.readVarint(true)
                ...
                8, 11, 14 -> { // Translate, Scale, Shear (2 floats)
                    val boneIndex = reader.readVarint(true)
                    val frameCount = reader.readVarint(true)
                    val bezierCount = reader.readVarint(true)
                    for (f in 0 until frameCount) {
                ...
                9, 10, 12, 13, 15, 16 -> { // TranslateX/Y, ScaleX/Y, ShearX/Y (1 float)
                    val boneIndex = reader.readVarint(true)
                    val frameCount = reader.readVarint(true)
                    val bezierCount = reader.readVarint(true)
                    for (f in 0 until frameCount) {
                ...
                18 -> { // Transform
                    ...
                    for (f in 0 until frameCount) {
                        val time = reader.readFloat()
                        for (v in 0 until 6) reader.readFloat()
                        if (time > maxDuration) maxDuration = time
                    }
                    readBeziers(reader, bezierCount, isVersion41Plus)
                }
                19, 20 -> { // Path position / spacing
                    val pathIndex = reader.readVarint(true)
                    val frameCount = reader.readVarint(true)
                    val bezierCount = reader.readVarint(true)
                    for (f in 0 until frameCount) {
                ...
                22 -> { // Deform
                    val skinIndex = reader.readVarint(true)
                    val slotIndex = reader.readVarint(true)
                    readStringRef()
                    val frameCount = reader.readVarint(true)
                    val bezierCount = reader.readVarint(true)
                    for (f in 0 until frameCount) {
                        ...
                        if (end > 0) {
                            ...
                            for (v in 0 until end) reader.readFloat()
                        }
                        if (time > maxDuration) maxDuration = time
                    }
                    readBeziers(reader, bezierCount, isVersion41Plus)
                ...
                25 -> { // Event
                    ...
                    for (f in 0 until frameCount) {
                        val time = reader.readFloat()
                        val eventIndex = reader.readVarint(true)
                        val eventData = data.events.getOrNull(eventIndex)
                        reader.readVarint(false)
                        reader.readFloat()
                        if (reader.readBoolean()) reader.readRawString()
                        if (eventData?.audioPath != null) {
                            reader.readFloat() // volume
                            reader.readFloat() // balance
                        }
        ...
        return Animation38(name, if (maxDuration <= 0f) 1.0f else maxDuration)
    }

    private fun readBeziers(reader: BinaryStreamReader38, bezierCount: Int, isVersion41Plus: Boolean) {
        if (bezierCount <= 0) return
        val floatsToRead = if (isVersion41Plus) bezierCount * 4 else bezierCount * 18
        for (b in 0 until floatsToRead) {
            reader.readFloat()
        }
    }

    private fun readSkin4x(
        reader: BinaryStreamReader38,
        skinName: String,
        isDefaultSkin: Boolean,
        nonessential: Boolean,
        ...
        if (nonessential) {
            reader.readInt() // skin color RGBA in Spine 4.0 / 4.1
        }
        val boneCount = reader.readVarint(true)
        ...
        for (i in 0 until slotCount) {
            ...
            for (ii in 0 until attachmentCount) {
                val placeholderName = readStringRef() ?: ""
                val attachment = readAttachment4x(reader, slotIndex, placeholderName, nonessential, strings, isVersion41Plus)
                if (attachment != null) {
                    skin.setAttachment(slotIndex, placeholderName, attachment)
                }
            }
    ...
    private fun readAttachment4x(
        ...
        when (type) {
            0 -> { // Region
                ...
                val x = reader.readFloat() * scale
                val y = reader.readFloat() * scale
                val scaleX = reader.readFloat()
                val scaleY = reader.readFloat()
                val width = reader.readFloat() * scale
                val height = reader.readFloat() * scale
                val color = reader.readInt()
                if (isVersion41Plus) {
                    val hasSequence = reader.readBoolean()
                    if (hasSequence) {
                        reader.readVarint(true)
                        reader.readVarint(true)
                        reader.readVarint(true)
                ...
                val regionAtt = RegionAttachment38(name).apply {
                    this.path = path
                    this.x = x
                    this.y = y
                    this.scaleX = scaleX
            ...
            1 -> { // BoundingBox
                val vertexCount = reader.readVarint(true)
                readVertices4x(reader, vertexCount)
                if (nonessential) reader.readInt()
                return null
            }
            2 -> { // Mesh
                ...
                val color = reader.readInt()
                val vertexCount = reader.readVarint(true)
                val uvs = FloatArray(vertexCount * 2) { reader.readFloat() }
                val triangleCount = reader.readVarint(true)
                val triangles = ShortArray(triangleCount) { reader.readShort() }
                val (bones, vertices) = readVertices4x(reader, vertexCount)
                val hullLength = reader.readVarint(true) * 2
                if (isVersion41Plus) {
                    val hasSequence = reader.readBoolean()
                    if (hasSequence) {
                ...
                mesh.region = findRegionInAtlas(atlas, path, name)
                mesh.updateUVs()
                return mesh
            }
            3 -> { // LinkedMesh
            ...
            4 -> { // Path
                reader.readBoolean() // closed
                reader.readBoolean() // constantSpeed
                val vertexCount = reader.readVarint(true)
                readVertices4x(reader, vertexCount)
                val lengthsCount = vertexCount / 3
                for (i in 0 until lengthsCount) reader.readFloat()
                if (nonessential) reader.readInt()
                return null
            }
            5 -> { // Point
            ...
            6 -> { // Clipping
                reader.readVarint(true) // endSlotIndex
                val vertexCount = reader.readVarint(true)
                readVertices4x(reader, vertexCount)
                if (nonessential) reader.readInt()
                return null
    ...
    private fun readVertices4x(reader: BinaryStreamReader38, vertexCount: Int): Pair<IntArray?, FloatArray> {
        val verticesLength = vertexCount shl 1
        if (!reader.readBoolean()) {
            val vertices = FloatArray(verticesLength) { reader.readFloat() * scale }
            return Pair(null, vertices)
        }
        ...
        for (i in 0 until vertexCount) {
            ...
            for (ii in 0 until boneCount) {
                bonesList.add(reader.readVarint(true))
                weights.add(reader.readFloat() * scale)
                weights.add(reader.readFloat() * scale)
                weights.add(reader.readFloat())
            }
        }
        return Pair(bonesList.toIntArray(), weights.toFloatArray())
    }
}
...
class BinaryStreamReader38(private val input: InputStream) {
    ...
    fun readString(): String? {
        return try {
            ...
            if (byteCount <= 0 || byteCount > 2 * 1024 * 1024) return null
            ...
            if (total == 0) null else String(bytes, 0, total, StandardCharsets.UTF_8)
        } catch (_: Exception) {
            null
        }
    }
